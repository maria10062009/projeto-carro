// script.js - VERSÃO FINAL COM AUTENTICAÇÃO E FUNÇÕES GLOBAIS CORRIGIDAS
...
(function () {
    ...
    const PUBLIC_API_URL = API_BASE_URL;
    ...
    // ==========================================================================
    // CACHE DE ELEMENTOS DO DOM (Completo)
    // ==========================================================================
    ...
    // ==========================================================================
    // FUNÇÕES DE AUTENTICAÇÃO E LÓGICA DE EXIBIÇÃO
    // ==========================================================================
    ...
    async function handleLogin(e) {
        ...
        try {
            const response = await fetch(`${AUTH_API_URL}/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
            const data = await response.json();
            if (!response.ok) throw new Error(data.message);
            authToken = data.token;
            localStorage.setItem('garagem-token', authToken);
            showApp();
            await inicializarAppPosLogin();
        } catch (error) {
            adicionarNotificacao(error.message || 'Erro ao tentar fazer login', 'erro');
        }
    }
    ...
    async function handleRegister(e) {
        ...
        try {
            const response = await fetch(`${AUTH_API_URL}/register`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
            const data = await response.json();
            if (!response.ok) throw new Error(data.message);
            adicionarNotificacao(data.message, 'sucesso');
            toggleAuthForms();
            formRegister.reset();
            formLogin.querySelector('#login-email').value = email;
        } catch (error) {
            adicionarNotificacao(error.message || 'Erro ao tentar registrar', 'erro');
        }
    }
    ...
    function showApp() {
        ...
        if (!document.getElementById('logout-btn')) {
            ...
            logoutButton.className = 'botao-perigo';
            logoutButton.addEventListener('click', handleLogout);
            header.prepend(logoutButton);
        }
    }
    ...
    function toggleAuthForms(e) {
        ...
        toggleToRegister.classList.toggle('hidden');
        authTitle.textContent = formLogin.classList.contains('hidden') ? 'Registro' : 'Login';
    }
    ...
    async function carregarGaragem() {
        try {
            listaVeiculosDiv.innerHTML = '<p class="placeholder-text">Carregando seus veículos...</p>';
            const response = await fetch(GARAGEM_API_URL, { headers: getAuthHeaders() });
            if (response.status === 401) {
                adicionarNotificacao("Sessão expirada. Faça login novamente.", "aviso");
                handleLogout();
                return [];
            }
            if (!response.ok) throw new Error(`Falha ao buscar veículos (${response.status})`);
            const garagemDoServidor = await response.json();
            return garagemDoServidor.map(data => {
                try {
                    const id = data._id;
                    switch (data._tipoClasse) {
                        case 'CarroEsportivo': return new CarroEsportivo(data.modelo, data.cor, data.velocidadeMaxima, id, data.historicoManutencao, data.ligado, data.velocidade, data.turboAtivado);
                        case 'Caminhao': return new Caminhao(data.modelo, data.cor, data.capacidadeCarga, data.velocidadeMaxima, id, data.historicoManutencao, data.ligado, data.velocidade, data.cargaAtual);
                        default: return new Carro(data.modelo, data.cor, data.velocidadeMaxima, id, data.historicoManutencao, data.ligado, data.velocidade);
                    }
                } catch (error) {
    ...
    // Disponibiliza a função globalmente para ser usada pelas classes de veículos
    window.atualizarVeiculoNoServidor = async function (veiculo) {
        const id = veiculo.id;
        const { id: idRemovido, ...dadosParaAtualizar } = veiculo;
        try {
            ...
            adicionarNotificacao(`Falha ao salvar dados do ${veiculo.modelo}!`, "erro");
        }
    }

    async function fetchAndDisplayWeather({ city, lat, lon }) {
        try {
            weatherElements.currentDisplay.innerHTML = '<p class="placeholder-text">Carregando tempo atual...</p>';
            const locationQuery = city ? `city=${city}` : `lat=${lat}&lon=${lon}`;
            const response = await fetch(`${PUBLIC_API_URL}/weather?${locationQuery}`);
            ...
            adicionarNotificacao(error.message, 'erro');
            weatherElements.cityName.textContent = "Erro!";
            weatherElements.currentDisplay.innerHTML = `<p class="error-text">${error.message}</p>`;
    ...
    async function buscarEExibirVeiculosDestaque() {
        try {
            cardsVeiculosDestaqueEl.innerHTML = `<p class="placeholder-text">Carregando destaques...</p>`;
            const response = await fetch(`${PUBLIC_API_URL}/veiculos-destaque`);
    ...
    async function buscarEExibirServicos() {
        try {
            listaServicosOferecidosEl.innerHTML = `<li class="placeholder-text">Carregando serviços...</li>`;
            const response = await fetch(`${PUBLIC_API_URL}/servicos-oferecidos`);
    ...
    function abrirModalEdicao(id) {
        ...
        if (ehCaminhao) { editModal.capacidadeInput.value = veiculo.capacidadeCarga; }
    ...
    // Tornada global para ser acessível pela classe Carro
    window.atualizarDisplayVeiculo = function () {
    ...
    function selecionarVeiculo(id) {
        ...
        if (!document.getElementById('tab-button-details').disabled) {
    ...
    // Tornada global para ser acessível pela classe Carro
    window.adicionarNotificacao = function (mensagem, tipo = 'info', duracaoMs = 5000) {
    ...
    // Tornada global para ser acessível pela classe Carro
    window.tocarSom = function (somId) {
    ...
    function setupEventListeners() {
        ...
        formAdicionarVeiculo.addEventListener('submit', async (e) => {
            ...
            try {
                const requestBody = { modelo, cor, _tipoClasse: tipo };
                if (tipo === 'Caminhao') { requestBody.capacidadeCarga = parseInt(capacidade, 10); }
                const response = await fetch(GARAGEM_API_URL, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(requestBody) });
                if (!response.ok) { const errData = await response.json(); throw new Error(errData.details || 'Falha ao adicionar veículo.'); }
                const veiculoSalvo = await response.json();
                garagem = await carregarGaragem();
                atualizarListaVeiculosUI();
                adicionarNotificacao(`${veiculoSalvo.modelo} adicionado!`, 'sucesso');
                formAdicionarVeiculo.reset();
                campoCapacidadeCarga.classList.add('hidden');
                selecionarVeiculo(veiculoSalvo._id);
        ...
        listaVeiculosDiv.addEventListener('click', async (e) => {
            ...
            if (target.matches('.action-delete')) {
                ...
                try {
                    const response = await fetch(`${GARAGEM_API_URL}/${id}`, { method: 'DELETE', headers: getAuthHeaders() });
        ...
        editModal.form.addEventListener('submit', async (e) => {
            ...
            const veiculo = garagem.find(v => v.id === id);
            if (!veiculo) return;
            const dadosAtualizados = { modelo: editModal.modeloInput.value.trim(), cor: editModal.corInput.value };
            if (veiculo instanceof Caminhao) { dadosAtualizados.capacidadeCarga = parseInt(editModal.capacidadeInput.value, 10); }
            try {
                const response = await fetch(`${GARAGEM_API_URL}/${id}`, { method: 'PUT', headers: getAuthHeaders(), body: JSON.stringify(dadosAtualizados) });
                if (!response.ok) throw new Error("Falha ao atualizar o veículo.");
                garagem = await carregarGaragem();
                atualizarListaVeiculosUI();
                adicionarNotificacao(`${dadosAtualizados.modelo} atualizado!`, 'sucesso');
                fecharModalEdicao();
                if (veiculoSelecionadoId === id) { atualizarDisplayVeiculo(); }
            } catch(error) { adicionarNotificacao(`Erro: ${error.message}`, 'erro'); }
        });

        editModal.closeBtn.addEventListener('click', fecharModalEdicao);
        editModal.backdrop.addEventListener('click', (e) => { if (e.target === editModal.backdrop) fecharModalEdicao(); });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        toggleToRegister.addEventListener('click', toggleAuthForms);
        toggleToLogin.addEventListener('click', toggleAuthForms);
        const tokenSalvo = localStorage.getItem('garagem-token');
        if (tokenSalvo) {
    ...
    async function inicializarAppPosLogin() {
        console.log("LOG: Inicializando Garagem Inteligente (Autenticado)...");
        setupEventListeners();
        garagem = await carregarGaragem();
        atualizarListaVeiculosUI();
        atualizarDisplayVeiculo();
        switchTab('tab-garage');
        await buscarEExibirVeiculosDestaque();
        await buscarEExibirServicos();
        await fetchAndDisplayWeather({ city: 'Sao Paulo' });
        adicionarNotificacao("Bem-vindo de volta à sua Garagem!", "info");
    }
